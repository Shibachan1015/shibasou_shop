defmodule ShibasouShop.Repo.Migrations.Phase1Core do
  ...
  def change do
    ...
    create constraint(:products, :products_status_chk, check: "status IN ('draft','active','archived')")
    create constraint(:products, :products_inventory_mode_chk, check: "inventory_mode IN ('assemble_on_order','preassembled')")
    ...
    create table(:variants) do
      ...
      add :option_values, :map # snapshot of selected option values, e.g., %{"Size" => "M"}
      add :weight_g, :integer
      add :status, :string, null: false, default: "active" # active | archived
      timestamps()
    end
    ...
    create table(:media_assets) do
      add :product_id, references(:products, on_delete: :delete_all), null: false
      add :kind, :string, null: false, default: "image" # image | video (future)
    ...
    create table(:components) do
      ...
      add :kind, :string, null: false # "hanao" | "ten" | "sole"
      add :size, :string
      add :attributes, :map
      timestamps()
    end
    ...
    create constraint(:component_stocks, :component_stocks_nonneg_chk, check: "qty_on_hand >= 0 AND qty_reserved >= 0")
    ...
    create table(:variant_components, primary_key: false) do
      ...
      add :size_map, :map # optional mapping for size-dependent components
    end
    ...
    create constraint(:inventory_levels, :inventory_levels_nonneg_chk, check: "qty_on_hand >= 0 AND qty_reserved >= 0")

    # Collections and product membership
    create table(:collections) do
      add :name, :string, null: false
      add :handle, :string, null: false
      add :type, :string, null: false, default: "manual" # manual | auto (future)
      add :rules, :map
      timestamps()
    end
    ...
    create table(:orders) do
      ...
      add :status, :string, null: false, default: "pending" # pending | placed | paid | fulfilled | cancelled
      add :totals, :map
    ...
    create constraint(:orders, :orders_status_chk, check: "status IN ('pending','placed','paid','fulfilled','cancelled')")
